...
if TYPE_CHECKING:
    from macropay_sdk import EventMetadataInput, EventsModelTypedDict

    from .strategies._base import S

logger = logging.getLogger("macropay_sdk.ingestion")


class Ingestion:
    """
    Event ingestion client for Macropay.

    This class handles the ingestion of events into the Macropay API without blocking
    ...
    def __init__(
        ...
        *,
        ...
        self._client = self._stack.enter_context(
            Macropay(access_token, server, server_url)
        )
